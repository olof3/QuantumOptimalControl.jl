...
#Jfinal, dJfinal_dx = QuantumOptimalControl.setup_infidelity_zcalibrated(Q_css*css_target)
Jfinal, dJfinal_dx = QuantumOptimalControl.setup_infidelity_zcalibrated(Q_css*css_target, :basic)

#QuantumOptimalControl.setup_infidelity_zcalibrated(Q_css*css_target, :basic)
...
t = LinRange(0, tgate, nsegments + 1) # t = 0:Δt:tgate
...
μ_state = 1e-2 * Δt/tgate # weighting factors
...
f, g, f_grad, g_jac, nu, ng, nx, nc, cache = setup_ipopt_callbacks(A0Δt, A1Δt, A2Δt, x0, zeros(2,nsegments), (Jfinal,dJfinal_dx), (L,dL_dx), B)

# Not quite the rabi rate since its on the coefficients
max_rabi_rate = 2π * 0.030
...
#nsplines = 15
#c0 = [0.05*ones(nsplines); zeros(nsplines)][:] + 0.03randn(nsplines,2)[:]
c0 = [0.01*ones(nsplines); zeros(nsplines)][:]# + 0.03randn(nsplines,2)[:]

prob = createProblem(nc, c_L, c_U, ng, g_L, g_U, ng*nc, 0, f, g, f_grad, g_jac)
prob.x .= c0 # c_opt[:]

addOption(prob, "hessian_approximation", "limited-memory")
addOption(prob, "max_iter", 250)
...
plot_2qubit_evolution(qb, t, x, u_opt/2π, max_rabi_rate=max_rabi_rate/2π)

## Plot the guard state population
plot_2qubit_evolution(qb, t, x, u_opt/2π, to_states=["20", "21", "22"], max_rabi_rate=max_rabi_rate/2π)

...
@time x, λ, dJdu = QuantumOptimalControl.grape_naive(A0, [A1, A2], Jfinal, u[:,1:100], x0, ; dUkdp_order=3)
...
#Nt = size(u, 2)
Nt = 100
cache = ([similar(x0) for k=1:Nt+1], [similar(c2r(x0)) for k=1:Nt+1], Matrix{real(eltype(A0))}(undef, 2, Nt))

@time sol = propagate_pwc(dxdt, x0, u[:,1:Nt], 1.0, cache)

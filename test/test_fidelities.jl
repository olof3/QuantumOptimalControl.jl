...
@test J(m, θ_opt) ≈ abs_sum_phase_calibrated(m) atol=1e-12
...
abs_sum_phase_calibrated(m, :lms_phase_semiold)
...
@test J(m, θ_opt) ≈ abs_sum_phase_calibrated(m) atol=1e-8
...
@test J(m, θ_opt) ≈ 3.365883939061934 
...
@test J(m, θ_opt) ≈ 2.9787244710195484
